...
def check_temp_list(temp: List[Dict[str, Any]], ancestor_median: int, index_from: int) -> int:
    """
    ...
    for i in range(len(temp)):
        if temp[i] is not None and temp[i]["for_which_genome"] == ancestor_median and \
                temp[i]["index_from"] == index_from:
            return i

    return -1


def count_cycle_look_ahead(ancestor_choice_structure: Optional[Dict[str, Any]]) -> int:
    """
    ...
    if ancestor_choice_structure["genome_1_path"]["tail"] < 0 and \
        ...
        return 3
    elif ancestor_choice_structure["genome_1_path"]["tail"] < 0 and \
            ancestor_choice_structure["genome_2_path"]["tail"] < 0:
        return 2
    elif ancestor_choice_structure["genome_1_path"]["tail"] < 0 and \
            ancestor_choice_structure["genome_3_path"]["tail"] < 0:
        return 2
    elif ancestor_choice_structure["genome_2_path"]["tail"] < 0 and \
            ancestor_choice_structure["genome_3_path"]["tail"] < 0:
        return 2
    elif (
...
class SmallPhylogeny:
    ...
    def get_priority_count(self, median_index: int, choice_structure_index: int) -> int:
        ...
        """
        result: int = 200
        index_from: int = self.tree.medians[median_index]. \
            choice_structures[choice_structure_index]["index_from"]
        which_genome: int = self.tree.medians[median_index]. \
            choice_structures[choice_structure_index]["for_which_genome"]
        tail_of1: int = self.tree.medians[median_index]. \
            choice_structures[choice_structure_index]["genome_1_path"]["tail"]
        tail_of2: int = self.tree.medians[median_index]. \
            choice_structures[choice_structure_index]["genome_2_path"]["tail"]
        tail_of3: int = self.tree.medians[median_index]. \
            choice_structures[choice_structure_index]["genome_3_path"]["tail"]
        ...
        if self.tree.medians[median_index]. \
           ...
           choice_structures[choice_structure_index]["genome_1_path"]["genome_tail"]:
        ...
        if self.tree.medians[median_index]. \
           ...
           self.tree.medians[median_index]. \
           choice_structures[choice_structure_index]["genome_2_path"]["genome_tail"]:
    ...
    def calculate_case(self, median_index: int, choice_structure_index: int, index_from: int, tail: int) -> int:
        ...
        created_choice_structure: List[Dict[str, Any]] = self.get_new_choice_structure(median_index,
                                                                                       choice_structure_index,
                                                                                       index_from,
                                                                                       tail)
    ...
    def get_created_fragment(self, median_index: int, node1: int, node2: int) -> List[PGMFragment]:
        ...
        if node1 > 0 and node2 > 0:
            ...
            ancestor_line: PGMPath = PGMPath.create_pgm_path(node1, node2, 1, 1)
        ...
        if index_from > 0:
            ...
            if ancestor_median != -1:
                ...
                if index_in_temp == -1:
                    ...
                    ChoiceStructure.set_new_path(temp[index_in_temp], new_path1)

        if from_tail > 0:
            new_path2: PGMPath = PGMPath.create_pgm_path(from_tail, index_from, genome_tail, genome_from)
            ancestor_median: int = -1
            ...
            if ancestor_median != -1:
                ...
                if index_in_temp == -1:
                    ...
                    ChoiceStructure.set_new_path(temp[index_in_temp], new_path2)
